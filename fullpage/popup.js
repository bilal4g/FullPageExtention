...
(function () {
  ...
  function makePdf(canvas) {
    ...
    // Scale image to A4 width (595.28pt) with NO side margins - full width
    const pw = 595.28;
    const sc = pw / iW;
    const ch = iH * sc;
    const ph = ch; // No top/bottom margins either - content fills the page
    const sd = 'q ' + pw + ' 0 0 ' + ch + ' 0 0 cm /Img Do Q';
    let pdf = '%PDF-1.4\n'; const off = [];
  ...
  function makeMultiPagePdf(slices) {
    ...
    const imageInfos = slices.map(canvas => {
      ...
      const sc = pw / iW;
      const ch = iH * sc;
      const ph = ch; // No margins - full width/height
    ...
    for (let j = 0; j < imageInfos.length; j++) {
      ...
      const drawCmd = `q ${pw.toFixed(2)} 0 0 ${info.ch.toFixed(2)} 0 0 cm /Img Do Q`;
