/* SnapScroll Studio - editor engine (no API key, fully client-side) */
(function () {
  'use strict';
  ...
  let pageMeta = null;                 // math/text metadata from the captured page
  let settings = { tileH: 1400, scale: 1, includeTranscript: true, defaultFormat: 'png' };
  ...
  async function applySnapshot(dataUrl) { const img = await loadImage(dataUrl); work.width = img.width; work.height = img.height; wctx.clearRect(0, 0, work.width, work.height); wctx.drawImage(img, 0, 0); syncSizes(); redraw(); updateDims(); }
  ...
  function blurRegion(x, y, w, h) { if (w < 2 || h < 2) return; const tmp = document.createElement('canvas'); const scale = 0.06; tmp.width = Math.max(1, Math.round(w * scale)); tmp.height = Math.max(1, Math.round(h * scale)); const tctx = tmp.getContext('2d'); tctx.drawImage(work, x, y, w, h, 0, 0, tmp.width, tmp.height); wctx.save(); wctx.imageSmoothingEnabled = true; wctx.imageSmoothingQuality = 'low'; wctx.drawImage(tmp, 0, 0, tmp.width, tmp.height, x, y, w, h); wctx.restore(); }

  function drawCropOverlay() { clearOverlay(); if (!cropSel) return; const { x, y, w, h } = cropSel; octx.save(); octx.fillStyle = 'rgba(0,0,0,0.5)'; octx.fillRect(0, 0, overlay.width, overlay.height); octx.clearRect(x, y, w, h); octx.strokeStyle = '#6366f1'; octx.lineWidth = 2 / zoom; octx.strokeRect(x, y, w, h); octx.fillStyle = '#6366f1';[[x, y], [x + w, y], [x, y + h], [x + w, y + h]].forEach(([hx, hy]) => { const s = 8 / zoom; octx.fillRect(hx - s / 2, hy - s / 2, s, s); }); octx.restore(); }
  function applyCrop() { if (!cropSel || cropSel.w < 4 || cropSel.h < 4) { toast('Draw a crop area first'); return; } const { x, y, w, h } = cropSel; const nc = document.createElement('canvas'); nc.width = Math.round(w); nc.height = Math.round(h); nc.getContext('2d').drawImage(work, x, y, w, h, 0, 0, nc.width, nc.height); work.width = nc.width; work.height = nc.height; wctx.clearRect(0, 0, work.width, work.height); wctx.drawImage(nc, 0, 0); cropSel = null; syncSizes(); fitZoom(); redraw(); clearOverlay(); pushHistory(); updateDims(); toast('Cropped'); }
  ...
  function exportAs(type) {
    ...
    else if (type === 'pdf') { const bytes = makePdf(flatten('#fff')); download(URL.createObjectURL(new Blob([bytes], { type: 'application/pdf' })), 'snapscroll.pdf'); }
  ...
  function makePdf(canvas) {
    ...
    const after = pdf.slice(before.length); for (let i = 0; i < after.length; i++) arr[before.length + imgStream.length + i] = after.charCodeAt(i);
    return arr;
  }
  ...
  // ---- AI-Readable (free, client-side) ----
  function buildTranscript() {
    const m = pageMeta || {};
    const eqs = (m.math && m.math.equations) || [];
    const lines = [];
    lines.push('The following is text and math extracted directly from a web page that was screenshotted.');
    lines.push('Math is given in LaTeX between $ (inline) or $$ (display). Read the equations from this text, not from the image pixels.');
    lines.push('');
    if (m.url) lines.push('Source: ' + m.url);
    if (m.title) lines.push('Title: ' + m.title);
    lines.push('');
    if (eqs.length) {
      lines.push('## Equations (' + eqs.length + ')');
      eqs.forEach((eq, i) => {
        const body = (eq.latex && eq.latex.trim()) || (eq.text && eq.text.trim()) || '';
        const wrap = eq.type === 'display' ? ('$$ ' + body + ' $$') : ('$ ' + body + ' $');
        lines.push((i + 1) + '. ' + wrap);
      });
      lines.push('');
    }
    if (m.headings && m.headings.length) {
      lines.push('## Headings');
      m.headings.forEach((h) => lines.push('- (' + h.level + ') ' + h.text));
      lines.push('');
    }
    if (m.tables && m.tables.length) {
      lines.push('## Tables');
      m.tables.forEach((t) => lines.push('- ' + t.rows + 'x' + t.columns + ': ' + (t.preview || '').slice(0, 240)));
      lines.push('');
    }
    if (m.selection && m.selection.text) { lines.push('## Selected text'); lines.push(m.selection.text); lines.push(''); }
    if (!eqs.length && !(m.headings && m.headings.length)) {
      lines.push('(No structured text was found on the page. Use the attached image slices - they are cut small enough to read.)');
    }
    return lines.join('\n').trim();
  }
  function renderEqList() {
    const box = $('ai-eqs'); box.innerHTML = '';
    const eqs = (pageMeta && pageMeta.math && pageMeta.math.equations) || [];
    $('ai-badge').textContent = eqs.length + ' equation' + (eqs.length === 1 ? '' : 's');
    if (!eqs.length) { box.innerHTML = '<div class="eq"><span class="tag">no equations found on page</span></div>'; return; }
    eqs.slice(0, 80).forEach((eq, i) => {
      const d = document.createElement('div'); d.className = 'eq';
      const body = (eq.latex && eq.latex.trim()) || (eq.text && eq.text.trim()) || '(empty)';
      d.innerHTML = '<span class="tag">#' + (i + 1) + ' · ' + (eq.type || 'inline') + '</span><code></code>';
      d.querySelector('code').textContent = body;
      box.appendChild(d);
    });
  }
  function openAI() {
    ...
    renderEqList();
    $('ai-text').value = buildTranscript();
    $('ai-modal').hidden = false;
  }
  function sliceForAI() {
    const tileH = Math.max(400, parseInt(settings.tileH, 10) || 1400);
    const scale = Math.max(0.25, Math.min(3, parseFloat(settings.scale) || 1));
    const w = work.width, h = work.height, n = Math.ceil(h / tileH);
    let i = 0;
    (function next() {
      if (i >= n) { toast('Exported ' + n + ' slice' + (n === 1 ? '' : 's')); return; }
      const sy = i * tileH, sh = Math.min(tileH, h - sy);
      const t = document.createElement('canvas'); t.width = Math.round(w * scale); t.height = Math.round(sh * scale);
      t.getContext('2d').drawImage(work, 0, sy, w, sh, 0, 0, t.width, t.height);
      download(t.toDataURL('image/png'), 'ai-part-' + (i + 1) + '-of-' + n + '.png');
      i++; setTimeout(next, 350);
    })();
    if (settings.includeTranscript) {
      setTimeout(() => { download('data:text/plain;charset=utf-8,' + encodeURIComponent(buildTranscript()), 'ai-transcript.txt'); }, n * 350 + 200);
    }
  }
  ...
  async function pasteFromClipboard() { try { const items = await navigator.clipboard.read(); for (const it of items) { const type = it.types.find((t) => t.indexOf('image') === 0); if (type) { const blob = await it.getType(type); readFile(blob); return; } } toast('No image in clipboard'); } catch (e) { toast('Paste blocked - use Ctrl+V on the page'); } }
  ...
  function bind() {
    ...
    $('ai-copy').onclick = () => copyText($('ai-text').value, 'Transcript copied - paste it with your image');
    $('ai-slices').onclick = sliceForAI;
    ...
    $('btn-settings').onclick = () => { if (chrome.runtime.openOptionsPage) chrome.runtime.openOptionsPage(); else window.open(chrome.runtime.getURL('settings.html')); };
  ...
  function boot() {
    bind(); setTool('pan');
    chrome.storage.local.get(['ss_settings', 'ss_studio_image', 'ss_studio_meta'], (d) => {
      if (d && d.ss_settings) settings = Object.assign(settings, d.ss_settings);
      if (d && d.ss_studio_meta) { pageMeta = d.ss_studio_meta; const c = (pageMeta.math && pageMeta.math.count) || 0; if (c) $('status-math').textContent = '· ' + c + ' equations detected'; chrome.storage.local.remove('ss_studio_meta'); }
      if (d && d.ss_studio_image) { setFromSource(d.ss_studio_image); chrome.storage.local.remove('ss_studio_image'); }
    });
  }
