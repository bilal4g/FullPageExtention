...
(function () {
  ...
  let cropAR = null;
  let activeText = null;
  let settings = { scale: 2, tile: 1600, overlap: 90 };
  ...
  function opts() {
    return { stroke: $('opt-stroke').value, width: parseInt($('opt-width').value, 10) || 4, fill: $('opt-fill').checked, fillColor: $('opt-fillcolor').value };
  }
  function coords(e) {
  ...
  function fitZoom() {
    const pad = 64;
    const z = Math.min((stageWrap.clientWidth - pad) / work.width, (stageWrap.clientHeight - pad) / work.height, 1);
    applyZoom(z > 0 ? z : 1);
  }
  ...
  function rotate(dir) {
    const nc = document.createElement('canvas'); nc.width = work.height; nc.height = work.width;
    const c = nc.getContext('2d'); c.translate(nc.width / 2, nc.height / 2); c.rotate((dir > 0 ? 90 : -90) * Math.PI / 180); c.drawImage(work, -work.width / 2, -work.height / 2);
    work.width = nc.width; work.height = nc.height; wctx.clearRect(0, 0, work.width, work.height); wctx.drawImage(nc, 0, 0);
  ...
  function styleTextEl(el) {
    ...
    el.style.fontFamily = $('opt-font').value; el.style.fontSize = (size * zoom) + 'px'; el.style.color = $('opt-textcolor').value;
    el.style.fontWeight = $('opt-bold').checked ? '700' : '400'; el.style.background = $('opt-textbg').checked ? 'rgba(0,0,0,0.55)' : 'transparent';
  }
  ...
  function flatten(bg) {
    const nc = document.createElement('canvas'); nc.width = work.width; nc.height = work.height;
    const c = nc.getContext('2d'); if (bg) { c.fillStyle = bg; c.fillRect(0, 0, nc.width, nc.height); } c.drawImage(work, 0, 0); return nc;
  }
  ...
  function exportAs(type) {
    ...
    else if (type === 'pdf') { const bytes = SSAIReady.makePdf([flatten('#fff')]); download(URL.createObjectURL(new Blob([bytes], { type: 'application/pdf' })), 'snapscroll.pdf'); }
  ...
  // ---- AI-Ready export (no API) ----
  function aireadSource() {
    if ($('airead-crop').checked && cropSel && cropSel.w > 4 && cropSel.h > 4) {
      const nc = document.createElement('canvas'); nc.width = Math.round(cropSel.w); nc.height = Math.round(cropSel.h);
      nc.getContext('2d').drawImage(work, cropSel.x, cropSel.y, cropSel.w, cropSel.h, 0, 0, nc.width, nc.height); return nc;
    }
    return work;
  }
  function aireadScale() { const el = document.querySelector('#airead-quality .chip.active'); return el ? parseInt(el.dataset.scale, 10) : (settings.scale || 2); }
  async function aireadExport(kind) {
    if (host.hidden) { toast('Load an image first'); return; }
    const status = $('airead-status'); status.textContent = 'Slicing…';
    await new Promise((r) => setTimeout(r, 30));
    const tiles = SSAIReady.buildTiles(aireadSource(), { scale: aireadScale(), maxTile: settings.tile || 1600, overlap: settings.overlap || 90 });
    if (kind === 'pdf') {
      const bytes = SSAIReady.makePdf(tiles);
      download(URL.createObjectURL(new Blob([bytes], { type: 'application/pdf' })), 'snapscroll-ai.pdf');
    } else {
      tiles.forEach((t, i) => setTimeout(() => download(t.toDataURL('image/png'), 'snapscroll-ai-' + (i + 1) + '.png'), i * 350));
    }
    status.textContent = tiles.length + ' tile(s) ready. Hand these to the AI.';
    toast('AI-ready: ' + tiles.length + ' tile(s)');
  }
  ...
  function bind() {
    ...
    $('btn-airead').onclick = () => openModal('airead-modal');
    document.querySelectorAll('#airead-quality .chip').forEach((c) => c.addEventListener('click', () => { document.querySelectorAll('#airead-quality .chip').forEach((x) => x.classList.remove('active')); c.classList.add('active'); }));
    $('airead-pdf').onclick = () => aireadExport('pdf');
    $('airead-tiles').onclick = () => aireadExport('tiles');
    ...
    $('btn-settings').onclick = () => chrome.tabs.create({ url: chrome.runtime.getURL('options.html') });
  ...
  async function pasteFromClipboard() {
    try { const items = await navigator.clipboard.read(); for (const it of items) { const type = it.types.find((t) => t.indexOf('image') === 0); if (type) { const blob = await it.getType(type); readFile(blob); return; } } toast('No image in clipboard'); }
    catch (e) { toast('Paste blocked - use Ctrl+V on the page'); }
  }

  function boot() {
    bind(); setTool('pan');
    chrome.storage.local.get(['ss_studio_image', 'ss_settings'], (d) => {
